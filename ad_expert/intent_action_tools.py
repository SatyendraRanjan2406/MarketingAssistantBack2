...
class IntentActionTools:
    ...
    def _enhance_with_rag(self, raw_data: List[Dict], action_name: str, query_params: Dict[str, Any], user_id: int = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                # Use Qdrant-based RAG service instead of Chroma-based
    ...
    def _get_qdrant_rag_context(self, action_name: str, raw_data: List[Dict], query_params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            # Check if Qdrant is available (try server first, then in-memory)
            try:
                client = QdrantClient("localhost", port=6333)
                client.get_collections()
                qdrant_available = True
                logger.info("Using Qdrant server")
            except Exception:
                try:
                    # Fallback to in-memory Qdrant
                    ...
                    try:
                        ...
                        logger.info("Created google_ads_docs collection in memory")
                        
                        # Add some sample data for testing
                        self._add_sample_rag_data(client, openai_client)
                    except Exception as create_error:
                        # Collection might already exist
                        logger.info(f"Collection creation info: {create_error}")
                        
                except Exception as memory_error:
                    logger.warning(f"Qdrant not available (server or memory): {memory_error}")
                    return {
                        "context": "Qdrant not available - using basic context",
                        "use_rag": False
                    }
            ...
            if not openai_api_key:
                logger.warning("OpenAI API key not available, skipping RAG context")
                return {
                    "context": "OpenAI API key not available - using basic context",
                    "use_rag": False
                }
            
            # Build query for RAG
            query = self._build_rag_query(action_name, raw_data, query_params)
            ...
            try:
                ...
                if results:
                    ...
                    context_parts = []
                    logger.info(f"Found {len(results)} RAG results with scores: {[r.score for r in results]}")
                    
                    for i, result in enumerate(results, 1):
                        if result.score > 0.3:  # Lower threshold for better recall
                            context_parts.append(f"{i}. {result.payload.get('text', '')} (Source: {result.payload.get('source', 'Unknown')})")
                    
                    if context_parts:
                        context = "Relevant Google Ads documentation:\n" + "\n".join(context_parts)
                        return {
                            "context": context,
                            "use_rag": True,
                            "sources": [r.payload.get('source', '') for r in results if r.score > 0.3]
                        }
                ...
                logger.warning(f"Qdrant search error: {search_error}")
                return {
                    "context": "Qdrant search error - using basic context",
                    "use_rag": False
                }
            
        except Exception as e:
            logger.error(f"Error in Qdrant RAG context: {e}")
            return {
                "context": "RAG context generation failed",
                "use_rag": False
            }
    ...
    def _add_sample_rag_data(self, client, openai_client):
        ...
        try:
            sample_docs = [
                {
                    "text": "Google Ads campaigns are the foundation of your advertising strategy. Each campaign has its own budget, settings, and targeting options. You can create different types of campaigns including Search, Display, Video, Shopping, and App campaigns.",
                    "source": "https://developers.google.com/google-ads/api/docs/get-started/introduction",
                    "section": "campaigns"
                },
                {
                    "text": "Ad groups contain your ads and keywords. They help you organize your ads by a common theme. Each ad group should focus on a specific product, service, or category.",
                    "source": "https://developers.google.com/google-ads/api/docs/get-started/introduction", 
                    "section": "ad_groups"
                },
                {
                    "text": "Keywords are the words or phrases that trigger your ads to show. Use relevant, high-intent keywords for better quality scores and performance. Include negative keywords to exclude irrelevant traffic.",
                    "source": "https://developers.google.com/google-ads/api/docs/get-started/introduction",
                    "section": "keywords"
                },
                {
                    "text": "Performance metrics include CTR (click-through rate), CPC (cost per click), conversion rate, and ROAS (return on ad spend). Monitor these metrics regularly to optimize your campaigns.",
                    "source": "https://developers.google.com/google-ads/api/docs/get-started/introduction",
                    "section": "performance"
                }
            ]
