...
from .message_builder import CustomerSelectionMessageBuilder, IntentMappingMessageBuilder, MessageBuilder
...
from google_ads_new.intent_mapping_service import IntentDataMappingResult, get_intent_mapping_service, get_intent_mapping_to_data_service
        ...
        def data_analysis_node(state: LangGraphState) -> LangGraphState:
            ...
            try:
                ...
                if not tool_results:
                    ...
                    5. Visual representations (describe charts/graphs that would be helpful)
...
5. **Visual Suggestions** - Describe specific charts/graphs that would enhance understanding
6. **Images** - Create images using OpenAI's image generation API
7. **Tabular Formatting** - Format data into professional tables using ChatGPT
8. **Data Visualization** - Create pie charts, bar graphs, line charts using ChatGPT
9. if none of the tools are needed or selected or if you think it is not needed or this query is not related to the tools, just say so and move on to the next step i.e. query the LLM (gpt-4o).

Use clear formatting with headers, bullet points, and structured sections.
Include specific metrics, percentages, PIE CHARTS , BAR CHARTS , LINE CHARTS , IMAGES , TABLES, and data points where available or needed for better explanation and understanding.
"""
...
- "Show my campaigns" → Use get_campaigns tool
- "How to improve CTR?" → Direct advice (no tools needed)
- "Analyze my performance" → Use tools + analysis
- "Create a poster" → Use generate_image tool
- "What's my budget?" → Use get_budgets tool
